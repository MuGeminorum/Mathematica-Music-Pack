(function ($) {
	...
	$.fn.tmTip = function (options) {
		...
		function tipInit($this, opts) {
			...
			if (isEmpty(content)) content = "Hint";
	...
	$.fn.tzTab.defaults = {
		...
		tabDatas: [
			{ title: "Option1", content: "111", "url": "user.action" },
			{ title: "Option2", content: "222", "url": "delete.action" },
			{ title: "Option3", content: "333" }
		]
	};
	...
	function initImgbox($this) {
		...
		$this.on(opts.eventType, function (e) {
			...
			$("body").append('<div class="tm-ui-overdisplay" title="Click to close image list"></div><div id="tm-box-wrap" style="display: block;  height: 468px;">' +
				'<div id="tm-box-outer">' +
				...
				'</div>' +
				'<div id="tm-box-title" style=""><span id="tm-box-tit">' + title + '</span><span id="tm-box-timer" style="margin:5px;"><a href="javascript:void(0)" id="tm-auto-play">Auto play</a></span></div>' +
			...
			$("#tm-auto-play").click(function () {
				var text = $(this).text();
				if (text == 'Auto play') $(this).text("Pause");
				if (text == 'Pause') {
					clearInterval(imgboxTimer);
					$(this).text("Auto play");
					return;
	...
	function tzImgboxLoading(src, opts) {
		...
		if (img.complete) {
			...
			img.onerror = function () {
				tmLoading("Failed to load or find image...", 1);
			}
		}
	};
...
$.tzPrompt = function (options) {
	...
	$dialog.find(".tzdialog_message").html("<div style='width:" + mw + "px;margin:0 auto;text-align:left'><p>&nbsp;&nbsp;" + opts.content + ":</p>" +
		"<p><input type='text'  autofocus='autofocus' style='width:" + mw + "px;height:25px;text-indent:0.5em;border:1px solid #ccc;' value='" + opts.value + "' placeholder='Please input context...'></p></div>");
	//这个确定按钮事件
	$dialog.find(".tzdialog_ok").off("click").on("click", function () {
		var $input = $dialog.find(".tzdialog_message").find("input");
		var value = $input.val().trim();
		if (value == "") {
			loading("Please input context!", 2);
			$input.focus();
			return;
		}
...
$.tzAjax = {
	...
	ajaxMain: function (opts, _url, dataJson) {
		$.ajax({
			...
			error: function () { loading("Sorry! The operation could not be responded on time, please try later...", 1); opts.error(); clearTimeout(ajaxTimeout); },
			success: function (data) {
				if (data.result == "logout") {
					loading("session out of time, please log in...");
				} else {
