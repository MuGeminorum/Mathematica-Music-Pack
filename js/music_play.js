...
var krAudio = {
	...
	//设置播放的音乐地址
	seturl: function () {
		if (this.Currentplay > this.allItem || this.Currentplay < 1) this.Currentplay = 1;
		var currentObject = $("#main-list .list-item").eq(this.Currentplay - 1);
		this.audioDom.src = getLink(currentObject, 4);
	},

	setpdf: function () {
		var el = $("#main-list .list-item").eq(this.Currentplay - 1);
	...
	ordermode: function () {
		...
		switch (krAudio.orderModes) {
			case 2:     // 单曲循环 -> 列表循环
				...
				orderDiv.attr("title", "List cycle");
...
function resizePDF() {
	if ($(document.body).width() < 895) {
		var sh = $(document.body).height();
		var fh = $(".footer").height();
		$("#music-stave").height(sh - fh - 35);
	}
	else {
		$("#music-stave").height($("#player").height() - 20);
	}
}
...
window.onresize = function () {
	isMobile = ($(document.body).width() < 895);
	resizePDF();
}
